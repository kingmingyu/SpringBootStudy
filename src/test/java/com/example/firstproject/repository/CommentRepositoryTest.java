...
class CommentRepositoryTest {
    ...
    void findByArticleId() {
        ...
        //Case 3: 9번 게시글의 모든 댓글 조회
        {
            ...
            List<Comment> expected = Arrays.asList();
            //4. 비교 및 검증
            assertEquals(expected.toString(), comments.toString(), "9번 게시글은 존재하지 않으므로 댓글X");
        }
        //Case 4: 999번 게시글의 모든 댓글 조회
        {
            ...
            List<Comment> expected = Arrays.asList();
            //4. 비교 및 검증
            assertEquals(expected.toString(), comments.toString(), "999번 게시글은 존재하지 않으므로 댓글X");
        }
        //Case 5: -1번 게시글의 모든 댓글 조회
        {
            ...
            List<Comment> expected = Arrays.asList();
            //4. 비교 및 검증
            assertEquals(expected.toString(), comments.toString(), "-1번 게시글은 존재하지 않으므로 댓글X");
        }
    }
    ...
    void findByNickname() {
        ...
        //Case 2: "Kim"의 모든 댓글 조회
        {
            ...
            List<Comment> comments = commentRepository.findByNickname(nickname);
            //3. 예상 데이터
            Comment a = new Comment(2L, new Article(4L, "당신의 인생 영화는?", "댓글 고고"), nickname, "아이 엠 샘");
        ...
        //Case 3: null의 모든 댓글
        {
            ...
            List<Comment> expected = Arrays.asList();
            //4. 검증 및 비교
            assertEquals(expected.toString(), comments.toString(), "nickname이 null값이므로 댓글을 찾을 수 없음");
        }
        //Case 4: ""의 모든 댓글
        {
            //1. 입력 데이터 준비
            ...
            List<Comment> expected = Arrays.asList();
            //4. 검증 및 비교
            assertEquals(expected.toString(), comments.toString(), "nickname이 없으므로 댓글을 찾을 수 없음");
        }
    }
}